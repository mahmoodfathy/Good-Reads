...
export class UserDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.subscriber = this.UserBooksService.getUserBooks().subscribe(
      (response: any) => {
  ...
  onUpdateList(event: any) {
    ...
    // });
    if (event.shelf == 'none') {
      newArr = this.userBooks.filter((bookObj) => bookObj.book._id != event.id);
    } else {
      newArr = this.userBooks.map((bookObj) => {
        if (bookObj.book._id == event.id) {
          return { shelf: event.shelf, book: { ...bookObj.book } };
    ...
    if (this.filterType != 'all') this.filterByShelf(this.filterType);
    else this.filteredArray = [...this.userBooks];
